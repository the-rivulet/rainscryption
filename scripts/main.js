let canvas;
let context;
function drawTrapezoid(img, dx, dy, t, b, yMult, flip = false) {
...
function clearCanvas() {
    ...
    // Floor
    let f1 = document.getElementById("floor1");
    let f2 = document.getElementById("floor2");
...
function drawBattleBoard() {
    let paw = document.getElementById("boardpaw");
    // Your side
    for (let i = 0; i < 4; i++) {
    ...
    // Incoming
    let inc = document.getElementById("incoming");
    for (let i = 0; i < 4; i++) {
        context.setTransform(1, 0, -0.55 + 0.5 * i, 1, 0, 0);
